...
export const subscribeSchedule = (config: ReturnType<typeof getConfig>) => {
  if (config.enable) {
    ...
    if (nowSeconds < startSeconds || nowSeconds >= endSeconds) {
      ...
      nowSeconds < endSeconds - endingHours * 3600
    ) {
      startCb();
    } else if (
      nowSeconds >= endSeconds - endingHours * 3600 &&
    ...
    scheduleJob(
      `0 ${Math.floor((startSeconds + endingHours * 3600) / 3600)} ${
        ((startSeconds + endingHours * 3600) % 3600) / 60
      } * * *`,
      () => {
        endingCb();
      }
...
function mySetInterval(fn: (arg?: any) => any, millisec: number) {
  function interval() {
    setTimeout(interval, millisec);
    fn();
  }
  return setTimeout(interval, millisec);
}
...
function showInputBox(uri: Uri) {
  window
    .showInputBox({
      ...
      prompt:
        "非工作时间必须要输入正确的工作区名称才能保存，否则会清空当前文件~",
    })
    .then((str) => {
      if (str) {
...
function addPreCommit(startSeconds: number, endSeconds: number) {
  if (workspace.workspaceFolders?.length) {
    const hooksUri = Uri.joinPath(
      workspace.workspaceFolders![0].uri,
      "/.git/hooks"
    );
    workspace.fs.writeFile(
      Uri.joinPath(hooksUri, "pre-commit"),
